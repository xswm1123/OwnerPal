...
- (void)alertView:(UIAlertView *)alertView willDismissWithButtonIndex:(NSInteger)buttonIndex;{
    if (buttonIndex != alertView.cancelButtonIndex) {
        alertView.submitSignal.tag = (int)buttonIndex;
...
- (void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex;{
    LKSignal *signal = [self.signlaDic objectForKey:[NSNumber numberWithInt:(int)buttonIndex]];
    if (!signal) {
...
-(void)addButtonWithTitle:(NSString *)title signalObject:(NSObject *)object;{
    int index =(int)[self addButtonWithTitle:title];
...
- (void)textFieldDidBeginEditing:(UITextField *)textField;{
    LKSignal *signal = [[LKSignal alloc]initWithSender:textField firstRouter:textField object:nil signalName:UITextField.BEGIN_EDITING tag:(int)textField.tag tagString:textField.tagString];
    [textField sendSignal:signal];
}

- (BOOL)textFieldShouldReturn:(UITextField *)textField{
    LKSignal *signal = [[LKSignal alloc]initWithSender:textField firstRouter:textField object:nil signalName:UITextField.RETURN tag:(int)textField.tag tagString:textField.tagString];
    [textField sendSignal:signal];
    return YES;
}
...
-(void)__textChanged:(NSNotification *)notification{
    if (notification.object == self) {
        LKSignal *signal = [[LKSignal alloc]initWithSender:self firstRouter:self object:nil signalName:[self class].TEXTCHANGED tag:(int)self.tag tagString:self.tagString];
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    ...
    if ([text isEqual:@"\n"]) {
        LKSignal *signal = [[LKSignal alloc]initWithSender:textView firstRouter:textView object:nil signalName:UITextView.RETURN tag:(int)textView.tag tagString:textView.tagString];
...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    LKSignal *signal = [[LKSignal alloc]initWithSender:textView firstRouter:textView object:nil signalName:UITextView.TEXTCHANGED tag:(int)textView.tag tagString:textView.tagString];
    [textView sendSignal:signal];
}

- (void)textViewDidBeginEditing:(UITextView *)textView;{
    LKSignal *signal = [[LKSignal alloc]initWithSender:textView firstRouter:textView object:nil signalName:UITextView.BEGIN_EDITING tag:(int)textView.tag tagString:textView.tagString];
    [textView sendSignal:signal];
}
...
- (void)actionSheet:(UIActionSheet *)actionSheet willDismissWithButtonIndex:(NSInteger)buttonIndex;{
    LKSignal *_signal = [[LKSignal alloc]initWithSender:self firstRouter:self.parentView object:self.date signalName:UIDatePicker.COMFIRM tag:(int)self.tag tagString:self.tagString];
    [self.parentView sendSignal:_signal];
}

-(void)dateChanged{
    LKSignal *_signal = [[LKSignal alloc]initWithSender:self firstRouter:self.parentView object:self.date signalName:UIDatePicker.DATECHANGED tag:(int)self.tag tagString:self.tagString];
    [self.parentView sendSignal:_signal];
}
...
- (void)actionSheet:(UIActionSheet *)actionSheet willDismissWithButtonIndex:(NSInteger)buttonIndex;{
    LKSignal *_signal = [[LKSignal alloc]initWithSender:self firstRouter:self.parentView object:nil signalName:UIPickerView.COMFIRM tag:(int)self.tag tagString:self.tagString];
    [self.parentView sendSignal:_signal];
}
