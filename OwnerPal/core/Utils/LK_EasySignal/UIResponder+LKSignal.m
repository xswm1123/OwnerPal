...
-(void)sendSignal:(LKSignal *)signal;{
    ...
    if ([signal.sender isKindOfClass:[UIView class]]) {
        NSString *handleSelector = [NSString stringWithFormat:@"handleLKSignal_%@_%@_TAG%ld:",signal.preName?signal.preName:signal.sender.className,signal.signalName,(long)((UIView *)signal.sender).tag];
        SEL sel = NSSelectorFromString(handleSelector);
        if ([self respondsToSelector:sel]) {
            [self performSelector:sel withObject:signal];
            return;
    ...
    if ([signal.sender isKindOfClass:[UIView class]]) {
        NSString *handleSelector = [NSString stringWithFormat:@"handleLKSignal_%@_TAG%ld:",signal.preName?signal.preName:signal.sender.className,(long)((UIView *)signal.sender).tag];
        SEL sel = NSSelectorFromString(handleSelector);
        if ([self respondsToSelector:sel]) {
            [self performSelector:sel withObject:signal];
            return;
